...
#            hi = n * m 
#            T_host = np.zeros(n*m).astype(np.float32)
            
            T = cuda.CudaNdarray.zeros((n*m,1))
            block_size = 16
            block0 = (block_size, block_size, 1)
            grid0 = (n/block_size+1, m/block_size+1,1)
            trans_kernel(A,T,np.intc(m),np.intc(n), block=block0, grid = grid0 );
            
           # T_host = np.array(A)
	   # cuda.memcpy_dtoh(T_host,T)
            """
	    T_host = np.array(T)
            A_host = np.array(A)
            At = np.reshape(A_host,(-1,n)).astype(np.float32)
            At = np.transpose(At).astype(np.float32)
            A_host = At.flatten().astype(np.float32)
            for i in range(m*n):
                if T_host[i] != A_host[i]:
                    print i

            A = cuda_ndarray.CudaNdarray(A_host)
	    ...
	    """
            ...
            gemm_kernel(T,B,C[0], np.intc(m), np.intc(n), np.intc(k), block= block, grid=grid)
